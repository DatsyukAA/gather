...
namespace Account.EventBus
{
    public class RabbitBus : IBus
    {
        ...
        public async Task SendExchangeAsync<T>(string exchange, T message, string exchangeType = "direct")
        {
            await Task.Run(() =>
            {
                ...
                properties.Persistent = false;
                var output = JsonConvert.SerializeObject(message);
                _channel.BasicPublish(exchange, string.Empty, null,Encoding.UTF8.GetBytes(output));
            });
        }
